...
$XMLSOURCE = 'http://'.$_SERVER['HTTP_HOST']."/frontrss/";
...
function parseRSS($xml)
{
    ...
    for($i=0; $i<$numItems; $i++) {
		...
		$entry->desc = crop(strip_tags($story->description,('<i><strong><em><b>')),150);
		...
		$entry->categoryCode = preg_replace(array('/\s|\s+/','/[^a-z\-]/','/\-{2,}/'),array('-','','-'),strtolower(html_entity_decode($story->category)));
