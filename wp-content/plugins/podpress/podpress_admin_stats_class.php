	...
	function settings_stats_edit() {
		...
		switch($_GET['display']) {
			case 'unqiuedownloaders':
				...
				echo '			<legend>'.__('The Unique Downloaders Per Episode', 'podpress').'</legend>'."\n";
				...
				$sql   = 'SELECT media, COUNT(DISTINCT remote_ip, user_agent) AS total '
				       . 'FROM '.$wpdb->prefix.'podpress_stats GROUP BY media ORDER BY total DESC LIMIT '.$start.', '.$limit.';';
				...
				echo '			<a href="'.$baseurl.'unqiuedownloaders&start='.$nextstart.'">Next '.$limit.' episodes</a>'."\n";
				echo '		</fieldset>'."\n";
				echo '	</div>';
				break;
			case 'rawstats':
